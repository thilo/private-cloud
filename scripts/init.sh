...
#   - the SETUP file   (ENV_FILE.setup)  — the initial admin-account secrets, loaded
#                                          only during provisioning / by the scripts
...
# Setup-only secrets -> the setup file: the initial admin-account passwords.
set_kv "$SETUP_FILE" IMMICH_ADMIN_PASSWORD  "$(gen 24)"
...
echo
echo "Admin credentials (stored in $SETUP_FILE — save them in Vaultwarden):"
grep -E '^(IMMICH_ADMIN_EMAIL|IMMICH_ADMIN_PASSWORD|SEAFILE_ADMIN_EMAIL|SEAFILE_ADMIN_PASSWORD)=' "$SETUP_FILE" \
  | sed 's/^/  /'
echo
